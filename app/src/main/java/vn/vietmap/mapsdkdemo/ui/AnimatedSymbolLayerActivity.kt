...
class AnimatedSymbolLayerActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        mapView.getMapAsync{
         map->   vietMapGL = map
            map.setStyle(VietMapTiles.instance.lightVector()) { style: Style? ->
                this.style = style
                setupCars()
                animateRandomRoutes()
                animateTaxi()
    ...
    private fun addRandomCars() {
        ...
        symbolLayer.withProperties(
            ...
            PropertyFactory.iconRotate(
                Expression.get(PROPERTY_BEARING)),
    ...
    private fun addPassenger() {
        ...
        style!!.addImage(
            PASSENGER,
            (ResourcesCompat.getDrawable(resources, R.drawable.ic_android_2, null) as BitmapDrawable).bitmap
        )
    ...
    private val latLngInBounds: LatLng
        get() {
            val bounds = vietMapGL.projection.visibleRegion.latLngBounds
